...
public class BPTreeInnerNode<T extends Comparable<T>> extends BPTreeNode<T>  implements Serializable
{
	...
	//gets the page of the first key greater than the inserted key
		public String getPage(T key) 
		{
			return children[findIndex(key)].getPage(key);
		}
		
		//updates reference of input key
		public void updateRef(T key,Ref recordReference,Ref oldr) 
		{
			children[findIndex(key)].updateRef(key,recordReference,oldr);
		}
